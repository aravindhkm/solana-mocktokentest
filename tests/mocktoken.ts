...
describe("mocktoken", () => {
  ...
  it("CloseAccount-Call", async () => {
    let user = accounts[3];
    let closeWallet = accounts[4];

    let getUserAssociatedAccount = await getAssociatedTokenAddress(mintKey,user.publicKey);
    let getCloserAssociatedAccount = await getAssociatedTokenAddress(mintKey,closeWallet.publicKey);
    let beforeUserTokenInfo = await getAccount(provider.connection,getUserAssociatedAccount);
    let beforeCloserTokenInfo = await getAccount(provider.connection,getCloserAssociatedAccount);

    expect(Number(beforeUserTokenInfo.amount)).equal(5e9);
    expect(Number(beforeCloserTokenInfo.amount)).equal(10e9);
    expect(beforeUserTokenInfo.closeAuthority.toBase58()).equal(closeWallet.publicKey.toBase58());
    ...
    try {
      await closeAccount(
        ...
        closeWallet,
        getUserAssociatedAccount,
        getCloserAssociatedAccount,
        closeWallet.publicKey
      );
    }catch (err) { }
    ...
    expect(Number(afterUserTokenInfo.amount)).equal(0);
    expect(Number(afterCloserTokenInfo.amount)).equal(15e9);
    ...
    let afterBalance = (await provider.connection.getAccountInfo(getUserAssociatedAccount));
    expect(afterBalance).equal(null);
  });
